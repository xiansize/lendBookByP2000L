...
public class MCReader {
        ...
        if (auth) {
            ...
            for (int i = firstBlock; i < lastBlock; i++) {
                try {
                    ...
                    blocks.add(NfcCommon.byte2HexString(blockBytes));
                } catch (TagLostException e) {
                    throw e;
                } catch (IOException e) {
            ...
            if (!useAsKeyB) {
                if (isKeyBReadable(NfcCommon.hexStringToByteArray(
                        ret[last].substring(12, 20)))) {
                    ret[last] = NfcCommon.byte2HexString(key)
    ...
    public int buildNextKeyMapPart() {
        ...
        if (mKeysWithOrder != null && mLastSector != -1) {
            ...
            boolean autoReconnect = NfcCommon.getPreferences().getBoolean(
     ...
     * ({@link NfcCommon#getOperationInfoForBlock(byte, byte, byte,
    ...
    public HashMap<Integer, HashMap<Integer, Integer>> isWritableOnPositions(
        ...
        for (int i = 0; i < keyMap.size(); i++) {
            ...
            if (pos.containsKey(sector)) {
                ...
                byte[][] acMatrix = NfcCommon.acBytesToACMatrix(ac);
                if (acMatrix == null) {
                    ret.put(sector, null);
                    continue;
                }
                boolean isKeyBReadable = NfcCommon.isKeyBReadable(
                ...
                for (int block : pos.get(sector)) {
                    if ((block == 3 && sector <= 31)
                        ...
                        int acValue = NfcCommon.getOperationInfoForBlock(
                                acMatrix[0][3],
                                acMatrix[1][3],
                                acMatrix[2][3],
                                NfcCommon.Operations.WriteAC,
                                true, isKeyBReadable);
                        // Is key A writable? (If so, key B will be writable
                        // with the same key.)
                        int keyABValue = NfcCommon.getOperationInfoForBlock(
                                acMatrix[0][3],
                                acMatrix[1][3],
                                acMatrix[2][3],
                                NfcCommon.Operations.WriteKeyA,
                                true, isKeyBReadable);
    ...
    public boolean setKeyFile(File[] keyFiles, Context context) {
        ...
        for (File file : keyFiles) {
            String[] lines = NfcCommon.readFileLineByLine(file, false, context);
            if (lines != null) {
                for (String line : lines) {
                    if (!line.equals("") && line.length() == 12
                            && line.matches("[0-9A-Fa-f]+")) {
                        try {
                            keys.add(NfcCommon.hexStringToByteArray(line));
